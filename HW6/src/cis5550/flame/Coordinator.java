...
class Coordinator extends cis5550.generic.Coordinator {
  ...
  public static KVSClient kvs;
    ...
    post("/submit", (request,response) -> {
      ...
      try {
        Loader.invokeRunMethod(jarFile, className, new FlameContextImpl(jarName, outputbuf, kvs, getWorkers(), args[1]), argVector);
      ...
      //gotta check if the length is zero
      if(outputbuf.length()==0){
        return "job is finished there is no ouptut";
      }
      else{
        String outputbufstring=outputbuf.toString();
        return outputbufstring;
      }
