...
public class FlameContextImpl implements FlameContext{
    ...
    public FlameRDD fromTable(String tablename, RowToString lambd) throws Exception{
        byte[] byteslambda=Serializer.objectToByteArray(lambd);
        String outputtablename=invokeOperation(tablename,"/context/fromTable", byteslambda, null);
        FlameRDD answer= new FlameRDDImpl(outputtablename, this);
        return answer;
    }
    ...
    //gonna copy paste my invoke operation and mkae some tweaks because now we taking in two tables
    String invokeJoinOperation (String inputtable1, String inputtable2, String oppath) throws Exception{
        String outputtable="rdd-"+System.currentTimeMillis()+"-"+tablecount;
        tablecount+=1;
        Partitioner part= new Partitioner(); //this is to help calculat ehte asissgments
        Vector<String> kvsworkers= new Vector<>(); //getting the list
        ...
        for (Partition p: parts){
            Thread t=new Thread(()->{
                try {
                    ...
                    String startkeyrange=p.fromKey;
                    String endkeyrange=p.toKeyExclusive;
                    String workerurl="http://"+assignedworker+oppath+"?in1="+inputtable1+"&in2="+inputtable2+"&out="+outputtable+"&kvs="+this.kvsmainaddress;
                    if(startkeyrange!=null){
                        workerurl+="&from="+URLEncoder.encode(startkeyrange,"UTF-8");
                    }
                    if(endkeyrange!=null){
                    ...
                    HTTP.Response workerresponse=HTTP.doRequest("POST",workerurl,null); //sending it in the request body
                    if (workerresponse.statusCode()!=200||!new String(workerresponse.body()).equals("OK")){
                        throw new Exception("worker at " +assignedworker+"failed");
                    }
                    synchronized (tasksuccess) {
    ...
    //copying and pasting invoke operaton again and gonna make some small changes aka to collect string results from workers instead of just  the ok
    String invokeFoldOperation (String inputtable, String oppath, byte[] serlambda, String zelement) throws Exception{
        String outputtable="rdd-"+System.currentTimeMillis()+"-"+tablecount;
        tablecount+=1;
        Partitioner part= new Partitioner(); //this is to help calculat ehte asissgments
        Vector<String> kvsworkers= new Vector<>(); //getting the list
        ...
        //gontta make one vector to keep trakc o the worker threads and one of to record if the task suceeded or failed
        Vector<Thread> workerthread=new Vector<>();
        Vector<String> workersuccess=new Vector<>(); 
        for (Partition p: parts){
            Thread t=new Thread(()->{
                try {
                    String assignedworker=p.assignedFlameWorker;
                    ...
                    synchronized (workersuccess) {
                        String rbody=new String(workerresponse.body());
                        workersuccess.add(rbody);
                    }
                } catch (Exception e) {
                    synchronized (workersuccess) {
                        workersuccess.add(null);
                    }
                }
            });
            //now gonna add the thread i just made to the list and star tit
        ...
        FlamePairRDD.TwoStringsToString lambda=(FlamePairRDD.TwoStringsToString) Serializer.byteArrayToObject(serlambda, new File(this.jarName));
        String finalresult=zelement;
        for(String r: workersuccess){
            if(r==null){
                throw new RuntimeException("fold worker failed");
            }
            finalresult=lambda.op(finalresult, r);
        }
        return finalresult;
